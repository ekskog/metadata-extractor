...
// Function to recursively get all image files (JPEG, HEIC, and AVIF)
function getAllImageFiles(dir, imageFiles = []) {
    ...
    for (const file of files) {
        ...
        if (stat.isDirectory()) {
            ...
            if (['.heic', '.heif', '.jpeg', '.jpg', '.avif'].includes(ext)) {
...
async function extractMetadata(imagePath, rootDir) {
    try {
        ...
        // Get relative path from root directory, but ensure folder name is included
        const relativePath = path.relative(rootDir, imagePath);
        const folderName = path.basename(rootDir);
        
        // If the relative path doesn't include the folder name, prepend it
        const sourceImagePath = relativePath.includes('/') ? relativePath : `${folderName}/${relativePath}`;
        ...
        const metadata = {
            sourceImage: sourceImagePath.replace(/\\/g, '/'),
            timestamp: "not found",
...
async function processImages(inputDir) {
    try {
        ...
        console.log(`Found ${imageFiles.length} JPEG/HEIC/AVIF files\n`);
        
        if (imageFiles.length === 0) {
            console.log('No JPEG, HEIC, or AVIF files found!');
            return;
        }
...
// Main execution
async function main() {
    console.log('Image Metadata Extractor (JPEG/HEIC/AVIF)');
    console.log('==========================================');
